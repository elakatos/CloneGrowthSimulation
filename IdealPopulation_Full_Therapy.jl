...
function tumourgrow_post_it(b0, d0, b_, d_, Tmax, startPopSize, cells, mutID, neoep_muts, fitnessChange)
    ...
    return Nvec, tvec, cells,nonimmvec, neoep_muts
end
...
for i=1:200
    ...
    NvecIT, tvecIT, cellsIT, immuneIT, neoep_mutsIT = tumourgrow_post_it(1, d0_it, 1, d_it, Tmax, popSize, cells, mutID, neoep_muts, fitnessChange)
    outITDFsim = DataFrame(t=tvecIT, N=NvecIT, nonImm=immuneIT )
    writetable("postIT_"*string(i)*".txt", outITDFsim)

    #Process mutations after 'immunotherapy'
