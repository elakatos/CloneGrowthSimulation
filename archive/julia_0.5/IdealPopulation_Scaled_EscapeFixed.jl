#=
    ...
    This file implements simulations with clonal active immune escape (if clonalEscape == true) and/or clonal strong neoantigens (if clonalNeoep == true).
    Parameters: d0, popSize, detLim, p, initial_mut, mu, neoep_dist, clonalNeoep, clonalEscape
    Outputs:
...
function start_population(p, neoep_dist,initial_mut, clonalEscape, clonalNeoep, allowNeoep=true, immThresh=0.5)
    mutID = 1
    N = 1
    cells = cancercell[]
...
function birthdeath_neoep(b0, d0, Nmax, p, neoep_dist, clonalEscape,clonalNeoep, initial_mut=10, mu=1, immThresh=0.5)

    dmax = d0 #dmax is updated throughout, starts from d0

    #initialize arrays and parameters
    cells, mutID, muts, nonimm, N = start_population(p, neoep_dist,initial_mut, clonalEscape, clonalNeoep)
    Nvec = Int64[]
    push!(Nvec,N)
    t = 0.0
    tvec = Float64[]
...
for i=1:100
    Nvec, tvec, mutID, muts, cells, immune = birthdeath_neoep(1, d0, popSize, p, neoep_dist, clonalEscape, clonalNeoep, initial_mut, mu);
